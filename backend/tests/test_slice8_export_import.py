...
def test_export_import_roundtrip_between_warehouses(client):
    ...
    created_item = client.post(
        f"/api/warehouses/{source_warehouse_id}/items",
    ...
    stock = client.post(
        f"/api/warehouses/{source_warehouse_id}/items/{source_item_id}/stock/adjust",
        json={"delta": 1, "command_id": str(uuid.uuid4())},
        headers=headers,
    )
    assert stock.status_code == 200

    exported = client.get(f"/api/warehouses/{source_warehouse_id}/export", headers=headers)
    ...
    imported = client.post(
        f"/api/warehouses/{target_warehouse_id}/import",
        json=snapshot,
        headers=headers,
    )
    ...
    target_items = client.get(f"/api/warehouses/{target_warehouse_id}/items", headers=headers)
    ...
    target_boxes = client.get(f"/api/warehouses/{target_warehouse_id}/boxes/tree", headers=headers)
