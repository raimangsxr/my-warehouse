...
def test_signup_login_and_create_warehouse(client):
    ...
    create_wh_res = client.post("/api/warehouses", json={"name": "Garaje"}, headers=headers)
    assert create_wh_res.status_code == 201
    warehouse = create_wh_res.json()

    list_res = client.get("/api/warehouses", headers=headers)
    assert list_res.status_code == 200
    payload = list_res.json()
    assert len(payload) == 1
    assert payload[0]["id"] == warehouse["id"]

    members_res = client.get(f"/api/warehouses/{warehouse['id']}/members", headers=headers)
    assert members_res.status_code == 200
    members = members_res.json()
    assert len(members) == 1

    tree_res = client.get(f"/api/warehouses/{warehouse['id']}/boxes/tree", headers=headers)
...
def test_forgot_reset_and_change_password_flow(client):
    ...
    final_login = client.post(
        "/api/auth/login", json={"email": "reset@example.com", "password": "finalpassword123"}
    )
...
def test_remember_me_sets_cookie_and_allows_cookie_refresh(client):
    ...
    refresh_res = client.post("/api/auth/refresh", json={"remember_me": True})
...
def test_remember_me_access_token_has_no_exp_and_is_revoked_on_logout(client):
    ...
    me_after_logout = client.get("/api/auth/me", headers={"Authorization": f"Bearer {access_token}"})
