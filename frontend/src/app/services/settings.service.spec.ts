...
describe('SettingsService', () => {
  ...
  it('loads llm settings for warehouse', () => {
    service.getLlmSettings('wh-1').subscribe((settings) => {
  ...
  it('updates smtp settings', () => {
    ...
    service.updateSmtpSettings('wh-1', payload).subscribe((settings) => {
  ...
  it('reprocesses item with llm fields', () => {
    service.reprocessItem('wh-1', 'item-1', ['tags']).subscribe((response) => {
      expect(response.item_id).toBe('item-1');
      expect(response.processed_fields).toEqual(['tags']);
    });

    const req = httpMock.expectOne(
      (request) => request.url === `${environment.apiBaseUrl}/settings/llm/reprocess-item/item-1`
    );
    expect(req.request.method).toBe('POST');
    expect(req.request.params.get('warehouse_id')).toBe('wh-1');
    expect(req.request.body).toEqual({ fields: ['tags'] });
    req.flush({
      message: 'ok',
      item_id: 'item-1',
      processed_fields: ['tags'],
      tags: ['tools'],
      aliases: []
    });
  });
